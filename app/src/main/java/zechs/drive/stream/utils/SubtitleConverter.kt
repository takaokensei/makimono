...
object SubtitleConverter {
    ...
    // Bump invalidates cached *_converted.srt files generated by the previous merge logic.
    private const val CONVERTER_VERSION = "v4"
    ...
    /**
     * Splits an ASS `Dialogue:` payload into its fields.
     * ASS format: Layer,Start,End,Style,Name,MarginL,MarginR,MarginV,Effect,Text
     *
     * Only the first [maxCol] commas outside `{...}` override blocks act as separators; everything after
     * the [maxCol]-th separator is the untouched remainder (the text column may itself contain commas).
     * The result therefore has `maxCol + 1` entries when the line is complete, so that `parts.size > maxCol`
     * holds for a valid line.
     */
    internal fun parseAssDialogueLine(payload: String, maxCol: Int): List<String> {
            ...
            when {
                ...
                char == ',' && !inBraces -> {
                    ...
                    if (result.size >= maxCol) {
                        // Use the index of THIS comma (not indexOf, which finds the first one in the line).
                        result.add(payload.substring(index + 1))
                        return result
                    }
                }
    ...
    /**
     * Resolves simultaneous/overlapping cues so that no two output cues are ever active at the same time.
     *
     * The timeline is split at every distinct start/end instant. Inside each elementary interval all
     * cues that are active there are stacked (in their original order, separated by a line break),
     * and adjacent intervals that end up with identical text are coalesced back together.
     *
     * Compared to only comparing neighbouring cues, this also handles partial overlaps and chains
     * (A overlaps B, B overlaps C) that would otherwise leave concurrent cues rendered on top of each
     * other by players that place every cue at the same fixed position.
     *
     * Input does not need to be sorted; the result is sorted by start time and overlap-free.
     */
    internal fun mergeOverlappingCues(events: List<TimedLine>): List<TimedLine> {
        if (events.size <= 1) return events

        // Stable order by start time keeps the original reading order for cues with equal start.
        val ordered = events.sortedBy { it.startMs }
        val cuts = ordered.flatMap { listOf(it.startMs, it.endMs) }.distinct().sorted()

        val result = mutableListOf<TimedLine>()
        for (i in 0 until cuts.size - 1) {
            val from = cuts[i]
            val to = cuts[i + 1]
            val active = ordered.filter { it.startMs <= from && it.endMs >= to }
            if (active.isEmpty()) continue

            val text = active.drop(1).fold(active.first().text) { acc, cue -> mergeText(acc, cue.text) }

            val last = result.lastOrNull()
            if (last != null && last.endMs == from && last.text == text) {
                result[result.size - 1] = last.copy(endMs = to)
            } else {
                result.add(TimedLine(from, to, text))
            }
        }
        return result
    }
     ...
     */
    internal fun mergeText(text1: String, text2: String): String {
        val lines1 = text1.split(LINE_BREAKS).map { it.trim() }.filter { it.isNotEmpty() }
        val lines2 = text2.split(LINE_BREAKS).map { it.trim() }.filter { it.isNotEmpty() }
    ...
    internal fun cleanAssText(rawText: String): String {
        var text = rawText
